...
async def tool_agentb_query(ctx: ToolContext, *, query: str, rerank: bool = False, where: Dict[str, Any] | None = None) -> Dict[str, Any]:
    where = where or {}
    run = ctx.store.get_run(ctx.corr_id)
    dispatch_counts = (run.get("context") or {}).get("dispatch_counts") or {}
    dispatch_counts["b_query_sent"] = int(dispatch_counts.get("b_query_sent", 0)) + 1
    ctx.store.update_context(ctx.corr_id, {"dispatch_counts": dispatch_counts})
    ...
    if not res.ok:
        run = ctx.store.get_run(ctx.corr_id)
        dispatch_counts = (run.get("context") or {}).get("dispatch_counts") or {}
        dispatch_counts["b_query_err"] = int(dispatch_counts.get("b_query_err", 0)) + 1
        ctx.store.update_context(ctx.corr_id, {"dispatch_counts": dispatch_counts})
    ...
    run = ctx.store.get_run(ctx.corr_id)
    dispatch_counts = (run.get("context") or {}).get("dispatch_counts") or {}
    dispatch_counts["b_query_ok"] = int(dispatch_counts.get("b_query_ok", 0)) + 1
    ctx.store.update_context(ctx.corr_id, {"dispatch_counts": dispatch_counts})
