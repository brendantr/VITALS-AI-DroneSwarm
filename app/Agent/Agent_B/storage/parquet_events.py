...
# NOTE: Added idempotency_key for ACP v0.2 traceability.
...
class ParquetEventsWriter:
    ...
    def normalize_message(self, msg: Dict[str, Any], mission_id: str, text_override: Optional[str] = None) -> Dict[str, Any]:
        ...
        if schema == "agentb.api.v0.1":
            return self.normalize_agentb_api_message(msg, mission_id=mission_id, text_override=text_override)
        return self.normalize_generic_message(msg, mission_id=mission_id, text_override=text_override)
    ...
    def normalize_mcp_message(self, msg: Dict[str, Any], mission_id: str, text_override: Optional[str] = None) -> Dict[str, Any]:
        ...
        text = text_override if (isinstance(text_override, str) and text_override.strip()) else _canonical_text_fallback(msg)
    ...
    def normalize_acp_message(self, msg: Dict[str, Any], mission_id: str, text_override: Optional[str] = None) -> Dict[str, Any]:
        schema = msg.get("schema")
        if schema != "acp.v0.2":
            raise ValueError(f"Expected schema 'acp.v0.2', got: {schema!r}")
        ...
        # ACP priority is in payload (ACP.Intent). Top-level priority typically doesn't exist.
        priority_raw = payload.get("priority", 50)
        priority = int(priority_raw) if isinstance(priority_raw, int) else 50
    ...
    def normalize_agentb_api_message(self, msg: Dict[str, Any], mission_id: str, text_override: Optional[str] = None) -> Dict[str, Any]:
        ts_raw = msg.get("ts")
        ts = _parse_iso_ts(ts_raw) or datetime.now(timezone.utc).astimezone(timezone.utc)
        date = _iso_date(ts)
        ...
        text = (
            ...
            else (msg.get("query") if isinstance(msg.get("query"), str) else _canonical_text_fallback(msg))
        )
    ...
    def normalize_generic_message(self, msg: Dict[str, Any], mission_id: str, text_override: Optional[str] = None) -> Dict[str, Any]:
        ts_raw = msg.get("ts")
        ts = _parse_iso_ts(ts_raw) or datetime.now(timezone.utc).astimezone(timezone.utc)
        date = _iso_date(ts)
    ...
    def append_messages(self, msgs: List[Dict[str, Any]], mission_id: str, texts: Optional[List[Optional[str]]] = None) -> int:
        """
        Append messages as normalized records.

        If `texts` is provided, it must be the same length as msgs and will be used as
        the `text` column value (without mutating msgs).
        """
        if texts is not None and len(texts) != len(msgs):
            raise ValueError("append_messages: if provided, texts must match msgs length")

        records: List[Dict[str, Any]] = []
        for i, m in enumerate(msgs):
            t_override = texts[i] if texts is not None else None
            records.append(self.normalize_message(m, mission_id=mission_id, text_override=t_override))

        self.append_records(records)
        return len(records)

    def append_mcp_messages(self, msgs: List[Dict[str, Any]], mission_id: str, texts: Optional[List[Optional[str]]] = None) -> int:
        if texts is not None and len(texts) != len(msgs):
            raise ValueError("append_mcp_messages: if provided, texts must match msgs length")

        records: List[Dict[str, Any]] = []
        for i, m in enumerate(msgs):
            t_override = texts[i] if texts is not None else None
            records.append(self.normalize_mcp_message(m, mission_id=mission_id, text_override=t_override))
